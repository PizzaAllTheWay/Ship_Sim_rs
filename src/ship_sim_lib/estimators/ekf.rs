// Custom library with all the utilities for estimators to make life easier
use crate::estimators::estimators_utils;
...
#[derive(Clone)]
pub struct SharedState<const N: usize> {
...
{   
    ...
    let (A, B) = estimators_utils::linearize_system(&f, &x_est_post, &u, 1e-4, 1e-4);
    ...
    let (F_d, B_d) = estimators_utils::discretize_ab_zoh::<NX, NU>(&A, &B, dt);
    ...
    F: Fn(&SVector<f32, NX>) -> SVector<f32, NY>,
{
    ...
    let H = estimators_utils::numerical_jacobian(&h, &x_est_pri, 1e-4);
